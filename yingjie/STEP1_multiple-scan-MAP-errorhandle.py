...
def acquire_matl_and_rename(excel, sheet, proxy, macro_name, sample_name, target_status,
                             OPO_power, IR_power, OPO_WAVELENGTH,
                             Original_Filename, New_Filename, Rescan, max_rescans=3):
    inputVal = {"executionType": "EXECUTION_TYPE_MATL"}
    if not wait_for_status_retry(
            ...
            retries=5,
            interval=10):
        print("Laser never became ready.")
        return
    ...
    while interrupted and rescan_count < max_rescans:
        ...
        wait_for_status_ok(excel, sheet, 'B39', macro_name, target_status=target_status)
    ...
    renamed_pairs = build_area_filenames(
        area_dir, 'oir', sample_name,
        OPO_WAVELENGTH[-1], OPO_power[-1], IR_power[-1], suffix)
    ...
    try:
        ...
        if mode == 'sweep':
            ...
            for i in tqdm.tqdm(range(sample + 1)):
                ...
                acquire_matl_and_rename(excel, sheet, proxy, macro_name, sample_name, "hold",
                                        OPO_power, IR_power, OPO_WAVELENGTH,
            ...
            for scan in parameters['scans']:
                ...
                acquire_matl_and_rename(excel, sheet, proxy, macro_name, sample_name, "OK",
                                        OPO_power, IR_power, OPO_WAVELENGTH,
                                        Original_Filename, New_Filename, Rescan)
